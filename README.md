...
Проект — песочница без цели и без проигрыша, поэтому «просто открыть» его
недостаточно: нужен маршрут. Их три, и все доступны из интерфейса.

**1. Посмотреть явления.** Кнопки в панели «Сцены» — это готовые опыты:
один нейрон даёт пилу потенциала на осциллографе, волна расходится по
решётке кольцом, кольцо работает генератором. В любой момент можно тянуть
мышью по сцене: под курсором нейроны получают удар током.

**2. Собрать свою сеть.** Панель «Своя сеть» строит сцену по вашим числам:
сколько нейронов, какая топология (случайная, слоистая, кольцо, решётка),
плотность и сила связей, доля торможения, фоновый вход. Меняйте по одному
параметру и смотрите, что станет с частотой и растровой диаграммой.

**3. Обучить и увидеть результат.** Панель «Опыт» запускает классический
протокол с учителем и показывает ИЗМЕРЕННЫЙ результат: сеть учат на паттерне
A, затем проверяют отклик на A против необученного B, до и после обучения.
Рядом считается контроль с выключенным обучением.

Кнопка **«Обучить по эпохам»** идёт дальше: обучение разбито на 100 шагов, и
после каждого измеряется ЛАТЕНТНОСТЬ — время до первого спайка читающего
слоя. Получается **кривая обучения**: видно разгон, насыщение и плато, а не
одно число. Сеть не просто «отвечает сильнее» — она узнаёт знакомый паттерн
**быстрее**.

![Кривая обучения: разгон и насыщение](docs/images/learning-curve.png)

> **Что на кадре.** 100 эпох обучения (по одному предъявлению). Латентность
> ответа на обученный паттерн A падает **11.0 → 5.0 мс** (быстрее на 6.0 мс),
> отклик на необученный B остаётся **11.0 мс**, контроль с выключенным
> обучением — **ровно 0 мс выигрыша**. Средний вес связей 3.000 → 7.496,
> обновлений STDP — 591 246.
>
> **Почему латентность, а не число спайков.** Первая версия мерила число
> спайков читающего слоя, и кривая получалась ЛЕСТНИЦЕЙ из трёх ступеней.
> Причина измерена: эта величина принимает ровно три значения (40, 80, 120)
> на 40 нейронах и не меняется при окне измерения 40, 100, 200 и 400 мс —
> слой отвечает синхронно. Ступени выдавали дискретность МЕРЫ за скачки
> обучения. Латентность непрерывна (время спайка интерполируется внутри
> шага) и физиологически осмысленна.

![Опыт: сеть научилась различать паттерны](docs/images/experiment.png)

> **Что на кадре (опыт «до/после»).** Сеть из 80 нейронов (40 входов →
> 40 читающих). После обучения отклик на паттерн A вырос с **0 до 70
> спайков**, отклик на необученный B остался **1**, разделение **+69**.
> Обновлений STDP — 252 364, средний вес связей 1.500 → 3.057. Контроль (тот
> же протокол с выключенным обучением) даёт разделение **ровно 0**.
>
> **Почему это главное, что здесь есть.** До этого проект показывал только,
> что веса МЕНЯЮТСЯ: счётчик обновлений, разброс весов. Измерено, что при
> границах STDP по умолчанию это ничего не значило — веса оставались в
> диапазоне 0.038…0.234 после миллиона обновлений, тогда как функциональный
> порог в этих сетях лежит около **веса 6**. Обучение работало и не влияло
> ни на что. Теперь результат измеряется откликом, а не активностью правила.
>
> **Сцена готовится к опыту — и это видно.** Опыт не работает «как есть» на
> произвольной сети, и причины оказались измеримыми: фоновый вход делает
> паттерн неразличимым (с пуассоновским входом читающий слой отвечает за
> 0.5 мс от ШУМА, а не от стимула), а вес может лежать ниже порога
> распространения (измерено: при весах 1, 2 и 4 отклика нет вовсе, при 8
> появляется). Поэтому перед опытом вход глушится, масштаб веса подбирается,
> а границы STDP приводятся к масштабу сцены. Отчёт говорит, что именно было
> сделано.
>
> **Чего опыт НЕ может — и говорит об этом прямо.** Сцены, которые
> разряжаются сами (кольцо, волна, рабочая память), для опыта не годятся:
> ответ там задаётся собственным ритмом, а не входом. Измерено: спонтанная
> активность 876…37 115 спайков за окно покоя против **ровно 0** у обучаемых
> сцен. Вместо случайных чисел панель сообщает причину и предлагает взять
> структуру «Слоистая».
...
| `npm.cmd test` | Юнит-тесты ядра (281 тест, ~16 с) |
| `npm.cmd run bench` | Замеры производительности ядра (5 замеров) |
| `npm.cmd run smoke` | Сквозная проверка в настоящем Chrome (45 проверок, включая мышь) |
| `npm.cmd run showcase` | Пересобрать витринные кадры `docs/images/*.png` (10 кадров) |
...
* **Границы весов задаются в масштабе сцены** (`Preset.stdpBounds`), а не одной
  константой на проект: базовые веса пресетов различаются на порядки (0.15 у
  разреженной сети, 200 у волны). Прежняя константа `wMax = 1` одновременно
  делала обучение безрезультатным на слабых сетях и разрушала сильные.
* **Измеренный результат обучения** (`core/experiment.ts`): сеть учат различать
  два паттерна по протоколу с учителем, затем измеряется отклик на обученный
  паттерн против необученного — плюс **контроль** с выключенным STDP. Измерено:
  разделение **+69** спайков против **ровно 0** в контроле.
* **Обучение по эпохам и кривая обучения** (`core/epoch-learning.ts`): 100
  шагов обучения, после каждого измеряется **латентность** ответа — время до
  первого спайка читающего слоя. Кривая показывает разгон и насыщение:
  измерено **11.0 → 5.0 мс** быстрее. Проба не влияет на обучение (STDP
  замораживается на время измерения), перед каждой точкой — пауза покоя.
* **Подготовка сцены к опыту**: глушится фоновый вход (иначе отклик порождён
  шумом, а не паттерном), подбирается масштаб веса, границы STDP приводятся к
  масштабу сцены. Отказ на самоподдерживающихся сценах — с объяснением.
...
* **Своя сеть**: сцена собирается по параметрам пользователя — число нейронов,
  топология, плотность и сила связей, доля и сила торможения, фоновый вход.
...
* **Панель «Опыт»**: обучение с учителем и измеренный результат — отклик на
  обученный паттерн против необученного, плюс контроль с выключенным STDP.
  Кнопка «Обучить по эпохам» строит **кривую обучения** по латентности ответа.
* **Кривая обучения** (Canvas2D): четвёртый прибор. Латентность ответа по
  эпохам для обученного паттерна A, необученного B и контроля — видно разгон,
  насыщение и то, что контроль стоит на месте.
* **Панель «Своя сеть»**: сборка сцены по параметрам — структура, размер,
  плотность и веса связей, торможение, фоновый вход.
* **Справка ведёт маршрутом, а не описывает интерфейс**: «с чего начать» тремя
  путями (посмотреть явления, собрать сеть, обучить) и отдельный раздел «чего
  здесь нет» — чтобы не искать MNIST и Hodgkin-Huxley.
...
src/
  core/        ЯДРО. Ничего не знает про DOM, Pixi и localStorage.
    ...
    experiment.ts     опыт обучения: протокол с учителем и измерение результата
    wave.ts           измерение волны: скорость фронта, профиль по кольцам
    memory.ts         удержание активности и поиск ритма
    presets.ts        восемь сцен как данные + сборка «своей сети»
  ...
  test/        Юнит-тесты ядра (19 файлов, 281 тест).
scripts/
  smoke.mjs        45 сквозных проверок в настоящем Chrome (6 — настоящей мышью)
  showcase.mjs     10 витринных кадров с проверкой «сцена не пуста»
...
**Результат обучения — отдельно от активности правила.** Обновления STDP
показывают, что правило считает; они НЕ показывают, что сеть научилась.
Измерено, что при границах по умолчанию разница принципиальна: за 1 192 771
обновление веса остались в диапазоне 0.038…0.234, тогда как отклик читающего
слоя начинается около веса 6 — то есть поведение не менялось ни при каком
числе эпох.

Опыт с учителем (`core/experiment.ts`) измеряет именно результат:

| Величина | Значение |
|---|---|
| отклик на обученный паттерн A | 0 → **70** спайков |
| отклик на необученный B | **1** спайк |
| разделение A − B | **+69** |
| контроль (STDP выключен) | **ровно 0** |

Разделение считается как РАЗНИЦА РАЗНИЦ: `(после_A − после_B) − (до_A −
до_B)`. Это не украшение — проба оставляет после себя возбуждение, поэтому
вторая проба меряется на «разогретой» сети: на свежей сети `до_A = 0` при
`до_B = 8`, и без такой формы «различение» появлялось бы там, где сеть ничему
не учили. Перед каждой пробой теперь стоит пауза покоя 120 мс.
...
| 39 | **График частоты рисовал среднее за прогон вместо динамики** | Частота делила накопленные спайки на всё прошедшее время: измерено 19.8, 25.4, 30.6, 35.8, 40.9, 46.1 Гц по ходу прогона — рост был артефактом усреднения, а не разгоном сети. Выглядело при этом как настоящий график | Частота по скользящему окну 200 мс; тесты «мгновенная держит полку, накопленная отстаёт», «у гаснущей сети падает ниже 1 Гц», «окно видит всплеск сильнее среднего» |
| 40 | **Момент снятия стимула был зашит константой** | `LevelSession` записывал жёсткое «20 мс». С пресетами волны и памяти это совпадало, но у кольца стимул длится **1 мс** — уровень с ним получил бы окно памяти, сдвинутое на 19 мс, и часть удержания выпала бы молча | Длительность читается у пресета; тест «сессия использует длительность стимула ИЗ ПРЕСЕТА» |
| 41 | **Обучение меняло веса, но не могло изменить поведение** | Потолок STDP `wMax = 1` — абсолютное число, а функциональный порог в этих сетях ≈ **6**. Измерено: 1 192 771 обновление, веса остались в 0.038…0.234, поведение не менялось ни при каком числе эпох. Все проверки были зелёными: они следили за счётчиками, а не за откликом | `core/experiment.ts` измеряет отклик на обученный паттерн против необученного; тесты «сеть научается различать» и «контроль не даёт различения»; границы переехали в `Preset.stdpBounds` |
| 42 | **Границы STDP обрезали веса сильных сцен** | `buildScene` вычислял параметры STDP, но передавал в сеть только `enabled` — границы молча оставались умолчаниями. Измерено на волне: веса 22…178 обрезались до **0.76…1.00**, спайки за 100 мс падали с 118 016 до 31 584 | `setStdp` принимает полные параметры; тест «пресет „Волна“ не разрушается включением STDP» |
| 43 | **«Неясно, что с этим делать»** | Справка начиналась с таблицы приборов: описывала, что ВИДНО, но не что ДЕЛАТЬ. Проект — песочница без цели, поэтому описание интерфейса бесполезно по построению: непонятно, какую из восьми сцен нажать и зачем | Справка начинается с «С чего начать — три пути» и содержит раздел «Чего здесь нет»; `smoke`: «справка объясняет, ЧТО делать — тремя путями» |
| 44 | **«Собрать сеть» было невозможно** | Панель «Сеть» давала три ползунка (вес, торможение, вход), и все три правят УЖЕ СОБРАННУЮ сцену. Структуру выбрать было нельзя: ни нейронов, ни топологии, ни плотности связей | `customPreset()` + панель «Своя сеть»; `smoke`: ползунок на 600 настоящим событием, кнопка, замер 600 нейронов / 35 861 связи, кольцо — ровно 600 связей |
| 45 | **Слоистая «своя сеть» писала связи за пределы массива** | `layers` — это РАЗМЕРЫ слоёв, а задавалось `[count / 4, count]`: при 400 нейронах границы выходили `[0,100)` и `[100,500)`, максимальный индекс цели — **499**. Запись мимо длины не падает и не сообщает об ошибке: связи просто пропадали, а сеть выглядела рабочей | Обрезка последнего слоя и отбрасывание связей за границей; тест «ни одна связь не ведёт за пределы сети» по всем четырём топологиям — проверяет ИНДЕКСЫ, а не число связей |
| 46 | **Мера обучения принимала всего три значения** | Кривая по числу спайков читающего слоя была ЛЕСТНИЦЕЙ `40 40 40 … 80 80 80 … 120`: эта величина принимает ровно три значения и не меняется при окне измерения 40, 100, 200 и 400 мс — слой отвечает синхронно. Ступени выдавали дискретность меры за скачки обучения | Мера заменена на **латентность** ответа (время до первого спайка). Она непрерывна, потому что время спайка интерполируется внутри шага; измерено 9.8 → 5.0 мс гладкой кривой с насыщением |
| 47 | **Проба сама двигала обучение, а «до» для A и B было разным** | Две ошибки одного корня. (1) Проба подаёт стимул и крутит сеть, значит при STDP сама обучает: измерено расхождение кривых 40 → 120 против 40 → 80. (2) Проба оставляет возбуждение (τ проводимости 5 мс, следов 20 мс), поэтому вторая точка меряется на «разогретой» сети: на СВЕЖЕЙ сети `до_A = 0` при `до_B = 8` | (1) STDP замораживается на время пробы и восстанавливается; тест «проба не меняет веса». (2) Пауза покоя 120 мс перед каждой точкой; контроль даёт **ровно 0** вместо −8 |
| 48 | **Опыт работал только на специально подобранной сцене** | На произвольных «своих сетях» — нулевое различение, что выглядело как «обучение сломано». Причины измеримы: фоновый вход делает паттерн неразличимым (с пуассоновским входом читающий слой отвечает за 0.5 мс и 272 спайка ОТ ШУМА; без входа — 0), вес лежит ниже порога распространения (при 1, 2 и 4 отклика нет, при 8 есть), а границы STDP `wMax = 1` схлопывали веса кольца с 200 до **0.99** | `prepareForLearning`: глушит фоновый вход, подбирает наименьший множитель с откликом, приводит границы STDP к масштабу сцены (`4 × средний вес`). Проверено на семи сценах: четыре обучаются, контроль везде −0.0 мс |
| 49 | **Самоподдерживающуюся сеть нельзя обучить — но нужен внятный отказ** | На кольце опыт выдавал случайные числа (`0.2`, `58.8`, `0.1` мс) и вердикт «не научилась», отправляя пользователя крутить параметры. Причина: эти сцены разряжаются САМИ — спонтанных спайков 876…37 115 против **ровно 0** у обучаемых, и «узнавание паттерна» там не определено. Ловушка: у кольца ПЕРВАЯ проба даёт фон 0, активность раскручивается постепенно (0 → 876 → 1567) | Отдельный вердикт `selfOscillating` с причиной и советом; фон измеряется НЕСКОЛЬКО раз, берётся максимум. Тест «самоподдерживающуюся сцену отвергает с объяснением»; `smoke`: «опыт отказывает С ПРИЧИНОЙ» |
...
| **Своя сеть** | **Структура, число нейронов, плотность и вес связей, торможение, фоновый вход — и кнопка «Собрать сеть»** |
| **Опыт** | **«Прогнать опыт»: обучение с учителем и измеренный результат с контролем** |
...
Панель «Своя сеть» — наоборот, собирает сцену **заново** по заданным числам:
смена топологии требует перестройки матрицы связей, индексов CSR и буфера
задержек, поэтому она и происходит целиком, а не «на лету». Явлений эта
панель не обещает: при произвольных числах сеть может молчать, и это
...
npm.cmd test              # 281 юнит-тест ядра, ~16 с
npm.cmd run build         # сборка
npm.cmd run preview       # в отдельном окне
npm.cmd run smoke         # 45 проверок в настоящем Chrome
npm.cmd run showcase      # 10 витринных кадров с проверкой «сцена не пуста»
```
...
окно не ломает вёрстку; **опыт обучения даёт измеренное различение, а
контроль — нет**; **кривая обучения падает по эпохам и рисуется в приборах**;
**опыт отказывает с ПРИЧИНОЙ там, где он неприменим**; **«своя сеть»
собирается по параметрам**.
...
так сцена оставалась пассивной при 26 зелёных проверках (дефект 24). По той
же причине опыт и сборка сети проверяются НАЖАТИЕМ КНОПКИ и настоящим
событием `input` на ползунке, а не вызовом функции.
