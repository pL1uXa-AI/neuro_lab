...
> **Что на кадре.** Сеть из 80 нейронов (40 входов → 40 читающих). После
> обучения отклик на паттерн A вырос с **0 до 70 спайков**, отклик на
> необученный B остался **1**, разделение **+69**. Обновлений STDP —
> 252 364, средний вес связей 1.500 → 3.057. Контроль (тот же протокол с
> выключенным обучением) даёт разделение **ровно 0**.
>
...
> сеть могла разогреться от повторяющегося стимула. На кадре видно, что
> выключение обучения убирает эффект полностью.
...
| `npm.cmd test` | Юнит-тесты ядра (265 тестов, ~26 с) |
| `npm.cmd run bench` | Замеры производительности ядра (5 замеров) |
| `npm.cmd run smoke` | Сквозная проверка в настоящем Chrome (41 проверка, включая мышь) |
| `npm.cmd run showcase` | Пересобрать витринные кадры `docs/images/*.png` (9 кадров) |
...
src/
  ...
  test/        Юнит-тесты ядра (17 файлов, 265 тестов).
scripts/
  smoke.mjs        41 сквозная проверка в настоящем Chrome (6 — настоящей мышью)
  showcase.mjs     9 витринных кадров с проверкой «сцена не пуста»
...
npm.cmd test              # 265 юнит-тестов ядра, ~26 с
npm.cmd run build         # сборка
npm.cmd run preview       # в отдельном окне
npm.cmd run smoke         # 41 проверка в настоящем Chrome
npm.cmd run showcase      # 9 витринных кадров с проверкой «сцена не пуста»
```
...
контроль — нет**; **«своя сеть» собирается по параметрам**.
