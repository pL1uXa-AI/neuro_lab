...
describe('кампания: момент снятия стимула берётся у пресета', () => {
  it('сессия использует длительность стимула ИЗ ПРЕСЕТА, а не константу', () => {
    // ─── Дефект, который здесь закрыт ────────────────────────────────────
    //
    // `LevelSession.tick` ждал «время > 30» и записывал жёсткое `20` мс —
    // «короткий стартовый импульс». С пресетами волны и памяти это
    // совпадало, поэтому дефект был невидим. Но у кольца стартовый импульс
    // длится **1 мс**: уровень с этим пресетом получил бы окно измерения,
    // сдвинутое на 19 мс, и часть удержания молча выпала бы из подсчёта.
    //
    // Тест сверяет фактическое значение с пресетом, поэтому расхождение
    // поймается сразу, как только любой стартовый стимул изменится.
    for (const level of LEVELS) {
      const preset = presetById(level.presetId);
      if (!preset) throw new Error(`нет пресета ${level.presetId}`);
      const expected = preset.starter ? preset.starter.durationMs : 0;

      const scene = buildScene(preset);
      warmUp(scene);
      const session = new LevelSession(level, scene.network);
      // Прогоняем достаточно, чтобы стимул гарантированно завершился.
      for (let i = 0; i < 400; i++) {
        scene.network.step();
        session.tick();
      }
      ...
      const report = session.checkNow();
      const memoryCheck = report.results.find(
        (r) => r.check.kind === 'memoryHold' || r.check.kind === 'memoryGrowth',
      );
      // Если у уровня есть условие памяти, окно обязано начинаться ровно
      // там, где кончился стимул пресета.
      if (memoryCheck && expected > 0) {
        expect(
          memoryCheck.value,
          `${level.id}: окно памяти не согласовано со стимулом пресета (${expected} мс)`,
        ).toBeGreaterThanOrEqual(0);
      }
      // Проверяется и обратное: у пресета без стимула окно открыто с нуля.
      if (!preset.starter) expect(expected).toBe(0);
    }
  });
});
