...
export function layeredTopology(
  ...
  // ─── Почему размеры слоёв обрезаются ─────────────────────────────────────
  //
  // `layers` — это РАЗМЕРЫ слоёв, раскладываемые подряд от нуля. Если их
  // сумма больше `count`, индексы целей выходят за границу массивов, и связи
  // ведут на несуществующие нейроны.
  //
  // Опасность в том, что это НЕ падает: запись в типизированный массив мимо
  // длины молча ничего не делает, поэтому такая связь просто пропадает, а
  // сеть выглядит рабочей. Измерено на «своей сети» с `layers: [100, 400]`
  // при `count = 400`: максимальный индекс цели равнялся **499**.
  //
  // Поэтому последний слой обрезается так, чтобы сумма не превышала `count`.
  if (bounds.length > 0) {
    const last = bounds[bounds.length - 1];
    if (last[1] > count) last[1] = count;
  }

  const push = (from: number, to: number): void => {
    // Ни один конец связи не может выйти за пределы сети. Проверка дешёвая,
    // а молчаливая потеря связи в горячем цикле — нет.
    if (from === to) return;
